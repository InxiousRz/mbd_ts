...
class GraphGen2():
    ...
    def __init__(self, dates_a, dates_b, devices):
        print("owo")
        try:
            # dates_a = datetime.date(year=2020, month=8, day=24)
            # dates_b = datetime.date(year=2020, month=8, day=28)
            lives = False
            # devices = "PH_Sensor"
            ...
            datas = db.GetDataSample2(devicename=devices,recorddate=dates_a, recorddate2=dates_b)
            if len(datas) == 0:
            ...
            self.fig.suptitle(str('Data ' + str(self.GR_UsedSensor) + ' On : ') + str(dates_a) + " until " + str(dates_b), fontsize=14, fontweight='bold')
